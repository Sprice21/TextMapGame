...
def showTablePers():
    ...
    for a in tempTableObject[0]:
        ...
        for inX,letter in enumerate(pS):
            if letter != ' ':
                pygame.draw.rect(gameDisplay,black,(inX*11,aC*20+40,12,39))
        simpleText(pS,(0,20*aC+40),lightgray)
        c += 1; aC += 1
...
def mapBuild(version=1):
    ...
    squareFill((gridW//2-40,gridH//2-30),(gridW//2-27,gridH//2-18),'='); diamondFill((8,8),6,','); circleFill((80,15),18,"'"); circleFill((75,30),7,"'")
...
def screenReset(water):
    ...
    showTablePers()
    pygame.display.update()
def wholeTableConvert(inTable,mode=0):#0-full to normal, 1-normal to full
    if mode == 0:
        outTable = []
        for sY in range(sSectorGridH):
            sY *= gridH
            sectorRow = []
            for sX in range(sSectorGridW):
                out1 = []
                sX *= gridW
                for y in range(sY,sY+gridH):
                    row = []
                    for x in range(sX,sX+gridW):
                        row.append(inTable[y][x])
                    out1.append(row)
                sectorRow.append(out1)
            outTable.append(sectorRow[::-1])
        return outTable[::-1]
    elif mode == 1:
        c = 0
        for sY in range(sSectorGridH):
...
if startMode == 'gen':
    ...
    with open("saves\\"+"SaveFile"+str(startMode)+".dat",'rb') as file:
        table,inventoryDict,stockpileLootDict,seenSet,(gridW,gridH) = pickle.load(file)
        inventoryDict['car']=1
        backupSeenSet = copy.deepcopy(seenSet)
...
pygame.display.update()
def gameLoop():
    global seenSet, stockpileLootDict, inventoryDict, playerHealth, table, progressStore, backupSeenSet, keySet, transportMode, uX,uY,uSX,uSY
    keySet = set()
    uX = math.floor(gridW/2)
    uY = math.floor(gridH/2)
